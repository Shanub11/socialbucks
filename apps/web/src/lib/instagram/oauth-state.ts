// TARGET PATH: apps/web/src/lib/instagram/oauth-state.ts
//
// CSRF protection for the Instagram connect flow, plus a defence against
// account-grafting.
//
// The plain CSRF case is the usual double-submit: a random nonce goes into
// an httpOnly cookie and into the `state` parameter, and the callback
// requires them to match.
//
// The subtler attack this also closes: an attacker starts the flow with
// *their own* Instagram account, then gets a signed-in victim to load the
// resulting callback URL. Without binding, the victim's Creator row gets
// the attacker's Instagram account attached — and from then on the
// attacker's reel views drive the victim's payouts. Binding the initiating
// Clerk user into the signed state and re-checking it at the callback
// means a state minted for one account is useless in another's session.
...
export const OAUTH_COOKIE_PATH = '/api/auth/instagram';

const STATE_TTL_MS = 10 * 60 * 1000;
const STATE_KEY_INFO = 'socialbucks:instagram-oauth-state:v1';
